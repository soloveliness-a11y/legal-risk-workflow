...
1. **主干固定，分支有限**。工作流主干是 6 个阶段、4 个人工检查点，Agent 不能替审批人作最终风险判断。它可以在明确的分支点建议补充调查、补材料或终止，但主检查点必须由人放行。
...
对你的 Agent 说"启动新风控项目"（或使用 [commands/start-project.md](../commands/start-project.md)），提供项目名与材料路径即可开始；说"继续 {项目名}"可跨会话恢复。也可以单独触发任一能力技能（如"整理这份访谈转写稿"），不必走完整流程。
...
| 2 | 尽调执行 | dd-check、dd-interview、补充核查 | 核查表、纪要、pending 清单 | CP2 尽调完成确认 |
...
主检查点是阻断性的人工审批：未确认时技能停止等待，确认记录写入 project_log.md。阶段允许回环（复核不通过可回 Phase 2 补调查或 Phase 3 改报告），回环时已有产物不覆盖、版本共存。
...
维护者用 4 个内部历史项目对重写后的系统做过有限回归，覆盖早期、成长期、Pre-IPO 与追加轮次四类场景：基准发现 54 项中 52 项完整或部分覆盖、整项遗漏 2 项（均为低重要性）、无依据发现 0 项；AC1 累计拦下 5 处真实信息丢失。聚合口径与未充分验证项见 [EVALUATION.md](./EVALUATION.md)。

这些数字只说明该版本在有限样本上的测试情况，不代表一般准确率。投后检查、多场访谈综合和 CLI 适配器的真实运行样本仍然有限。首次使用建议先用自己的材料小规模对照验证。
