...
**V8.4（2026-09-18）— 核心原则表与范围边界**

- 风险评级核心原则表重写：删除潜在封顶/红线排除/常见降级行，改为评级依据（事实状态×影响×可能性×可缓释）、暂缓定级、行业特异性不受背景稀释
- 场景判断表后新增范围边界：用户限定基于现有材料出稿时优先遵守该范围，补查需求入待核实事项，不自动扩展为完整尽调

**V8.3（2026-09-18）— 入口分流：材料报告模式**

- 场景判断新增材料报告模式：用户要求基于现有材料出报告、未要求开展尽调时，不进尽调流程、不设 CP1/CP2（CONTRACT §4.1 检查点适用性），材料整理与证据登记后直接出带限制说明的 V0.1d 初稿，缺口入关键待核实事项；定稿/外发走 CP3；产出不得声称已完成尽调
- 异常表"跳过中间 Phase"改为场景化表述：材料报告模式是独立任务范围，不是对尽调路径的跳过

**V8.2（2026-09-18）— 检查点收敛：三阻断 + 一收口**

- CONTRACT §4 重构：CP1/CP3/CP4 保持人工审批阻断；CP2 由阻断性审批改为收口呈报（覆盖率+待核实清单默认随报告初稿一并交 CP3 审批，结论性缺口时阻断升级，由审批人决定补充/带限制继续/暂缓）
- 会话启动不再要求用户确认后才执行：无异常且无分支点时直接执行默认动作，存在异常或分支点才停下等待
- 增量模式补变更摘要交付（CONTRACT §8.1）；异常表"关键材料缺失>50%"对齐 CP2 结论性缺口处理
