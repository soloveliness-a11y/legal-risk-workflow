...
### 4.1 主检查点（3 个阻断性审批 + 1 个收口呈报）

| CP | 位置 | 类型 | 呈报内容 | 通过后动作 |
|----|------|------|---------|-----------|
| **CP1 尽调启动确认** | Phase 1 末 | 人工审批，阻断 | 风险假设清单 + 尽调清单（外发版）+ 访谈提纲 + 推断/缺省字段（投资阶段、类型等） | 清单可外发公司，进入现场尽调 |
| **CP2 尽调收口** | Phase 2 末 | 呈报，条件阻断 | 核查覆盖率 + 待核实清单 + 纪要完整性 | 默认不停顿：随报告初稿一并交 CP3 审批，直接进入报告撰写（V0.xd 带待补标注） |
| **CP3 报告定稿确认** | Phase 4 末 | 人工审批，阻断 | 报告（含 CP2 收口内容与关键待核实事项）+ 复核结论 + 修改闭环 | 报告封版，进入交易文件 |
| **CP4 交易与投后基线确认** | Phase 5 末 | 人工审批，阻断 | 条款覆盖度 + 前轮权利保全 | 进入投后管理 |

- **阻断性审批（CP1/CP3/CP4）**覆盖只有人能做的决定：批准外发文件与工作范围、接受未决风险并批准定稿、确认交易条件。必须由**审批人**（用户或其指定的角色）明确确认后才可继续；未确认时技能停止并等待。确认记录写入 `project_log.md` 一行：`[CP{n}] {日期} {审批人} 通过/有条件通过 意见摘要`。不设签名模板。
- **CP2 默认不停顿**："是否进入报告撰写"属机械性确认。尽调结束时产出收口呈报（覆盖率 + 待核实清单），随后直接撰写报告初稿，收口内容与关键待核实事项随初稿一并呈 CP3——材料能够带着明确限制继续形成初稿的，继续。
- **CP2 阻断条件（结论性缺口）**：缺口足以改变核心结论时（如关键主体材料整体缺失、访谈完全未成、重大事实冲突未裁决且直接影响定级），暂停并向审批人标明缺口与影响，由审批人决定补充材料、带限制继续或暂缓该项判断；触发与决定写 `project_log.md` 一行。
- **材料报告模式的检查点适用**：用户要求基于现有材料出报告、未要求开展尽调时（risk-workflow 场景判断），没有外发文件与尽调范围动作，CP1/CP2 不适用——审批点跟着实际动作走，不为生成一份草稿制造前置审批。路径：材料归档与预处理 → 证据登记 + Dossier 初始化 → risk-report 带限制说明的 V0.1d 初稿（缺口入关键待核实事项）→ report-review；定稿或对外发出走 CP3，接受重大未决风险仍由人决定。产出不得声称已完成尽调；结论性缺口突出标明并暂缓该项判断，交人决定。

### 4.2 子确认（默认随主检查点呈报）

技能内部的重要中间判断（风险假设成型、纪要单场完成、报告分析框架等）不单独停顿：默认并入下一个主 CP 的呈报项一并审批，记 `project_log.md` 一行。仅当该判断会改变工作范围或结论方向（如分析框架增删重大风险主题、清单范围重大调整）时，才单独向用户提出。不设编号体系、不设签名；未获呈报与审批的判断不得视为已确认。
...
- 敏感材料（合同/协议等交易文件、员工花名册、财务报表、资产与 IP 清单、访谈纪要与转写稿、外部尽调报告（律所/会所）、投资建议书/BP，以及任何含未公开项目事实或个人信息的材料）**默认禁止上传外部 OCR 服务**：仅可用宿主本地 OCR 能力，否则人工转录关键页或按 AC5 降级标注。例外（信任路径）：使用者已核验服务商保密义务且承诺不用于模型训练，并在 config 声明 `paddleocr.s1_policy: trusted-provider` 的，经逐次显式确认可上传。
- 已确认公开可获得的材料（上市公司公告、监管与政府公示文件、招股说明书、工商公示档案等）可经外部 OCR；来源公开但文件含未公开项目信息的，按敏感材料处理。调用脚本必须显式确认外传（`paddleocr_api.py --allow-external-upload`）。
...
任何研究类产出（行业研究/专项调研）必须落盘 `sources_*.json`（URL + 标题 + 访问日期）。**无来源不通过**（AC3 的研究类形态）。检索渠道不限：通用搜索、行业媒体、社区讨论均可。完全离线或禁用检索的运行环境：研究类产出按 AC5 降级——`research_*.md` 明确标注「离线降级」并列出待补检索项，`sources_*.json` 为空时须注明原因，不因无法检索阻断 Phase 推进；恢复检索能力后补跑并回填。
...
- 主检查点呈报的默认交付物三件：① 当前核心产物（清单 / 报告初稿 / 定稿报告 / 交易文件三件套）；② **关键待核实事项**（`pending_materials.json` 与 `pending_items.json` 的合并视图，按对结论的影响排序）；③ 增量执行时附**变更摘要**（受影响风险编号、等级变化、新增/关闭项及理由）。
- 证据底稿（Dossier、evidence_registry、核查表、研究文件、output.json 等）不逐个展开：呈报附一份产物索引（路径 + 一句话用途），审阅人按需查阅。
- 新材料、新转写到达时走**增量路径**：dd-check T3 定向核查 + 报告 V0.xd 迭代 + 批量更新校准，只更新受影响的判断并交付变更摘要，不重跑全流程；触发 §4.1 CP2 结论性缺口条件的除外。
...
| 日期 | 变更 |
|------|------|
| 2026-09-18 | 检查点收敛：CP1/CP3/CP4 保持人工审批阻断，CP2 由阻断性审批改为收口呈报（默认随报告初稿并入 CP3 审批，结论性缺口时阻断升级）；§4.2 子确认默认并入下一个主 CP 呈报，不再单独停顿；新增 §8.1 默认交付（核心产物 + 关键待核实事项 + 变更摘要，证据底稿按需查阅）与增量更新规则 |
