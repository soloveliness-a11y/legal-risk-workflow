...
| §4 检查点 | 核查范围/结果摘要并入 CP2 收口呈报（§4.1：覆盖率+待核实清单，默认随报告初稿一并交 CP3 审批，结论性缺口时阻断）；过程中不单独停顿确认 |
...
| T2 | 全量扫描+深度核查 | 按类型分类（工商/证照/合同/财务/知产/劳动等）；扫描件按 CONTRACT §7.2 处理（合同/花名册/财务/清单/尽调报告/访谈材料等 S1 类别禁止外传 OCR，走本地/人工/AC5 降级），结果存 `{文件简称}_ocr.md` |
